...
- **Milestone 2 — Camera capability detection.** Build `CameraCaptureService.capabilities()` and a `CameraCapabilities` model; surface it on a developer/debug screen. Zero dependency on backend or scoring work above — purely `Camera/` + `Diagnostics/` + minimal `UI/`.
...
Not started; no code from Milestones 2–10 was written in this session.
